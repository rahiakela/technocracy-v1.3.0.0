...
export class AuthController {
    ...
    /**
     *  Login the user by verifying password if the user account is activated
    ...
    public login(req: Request, res: Response, next: NextFunction) {
        ...
        User.findOne({"local.email": email})
            ...
            .then((user: any) => {
                ...
                let token = user.generateJWT(user._id, user.local.email, user.local.name);
                user.jwtToken = token;
                user.salt = "";
                user.hash = "";
    ...
    public activateAccount(req: Request, res: Response, next: NextFunction) {
        ...
        User.findOne({"local.activateToken.token" : verifyToken})
            .then((user: any) => {
                ...
                let expiredHours = DateTimeUtils.getExpiredTime(new Date(Date.now()), expires);
            ...
            .then((user) => {
                ...
                User.findOneAndUpdate(
                    ...
                    .then((updatedUser: any) => {
                        ...
                        res.json({"statusCode":200, "mailSent": true});
                        next();
                    })
                    .catch(next);
